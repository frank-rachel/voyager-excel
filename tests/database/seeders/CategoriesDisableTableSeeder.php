...
class CategoriesDisableTableSeeder extends Seeder
{
    /**
    ...
    {
        //Data Type
        $dataType = $this->dataType('name', 'categorydisables');
        if (!$dataType->exists) {
            $dataType->fill([
                'slug'                  => 'categorydisables',
                ...
                'model_name'            => 'FrankRachel\\VoyagerExcel\\Tests\\Models\\CategoryDisable',
        ...
        //Data Rows
        $categoryDataType = DataType::where('slug', 'categorydisables')->firstOrFail();
        ...
        $menuItem = MenuItem::firstOrNew([
            ...
            'url'     => '',
            'route'   => 'voyager.categorydisables.index',
        ]);
        if (!$menuItem->exists) {
            $menuItem->fill([
                'target'     => '_self',
        ...
        //Permissions
        Permission::generateFor('categorydisables');

        //Content
        $category = CategoryAllow::firstOrNew([
            'slug' => 'category-1',
        ]);
        if (!$category->exists) {
            $category->fill([
